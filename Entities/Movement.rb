class Movements < Entities
  def setup_data
	...
	def Movements.from_json( str )
		Movements.from_s( ActiveSupport::JSON.decode( str )["str"] )
	end
    
	def Movements.from_s( str )
		...
		our_m = nil
		if not ( our_m = Movements.find_by_global_id(global_id) )
			dputs 3, "New movement"
			our_m = Movements.create( desc, date, value, 
				a_src.id, a_dst.id )
	...
	def Movements.create( desc, date, value, source, dest )
